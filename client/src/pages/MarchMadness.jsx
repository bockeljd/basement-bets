...
import { Shield, Crosshair, Activity, AlertTriangle, Users, TrendingUp, Cpu, RefreshCw, Swords } from 'lucide-react';
...
const MarchMadness = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (allTeams.length > 0) {
                ...
                if (!selectedTeamB) {
                    const unc = allTeams.find(t => t.team_name.includes('North Carolina') || t.team_name === 'North Carolina') || allTeams[1];
                    setSelectedTeamB(unc);
                }
            }
        } catch (err) {
    ...
    useEffect(() => {
        if (isMatchupMode && selectedTeam && selectedTeamB) {
            fetchMatchup();
        }
    }, [isMatchupMode, selectedTeam, selectedTeamB]);
    ...
    return (
        <div className="bg-slate-950 min-h-screen text-white pb-20">
            {/* Action Bar */}
            <div className="flex justify-between items-center p-4 border-b border-slate-800 bg-slate-900/50 sticky top-0 z-50 backdrop-blur-md flex-wrap gap-4">
                <div className="flex items-center gap-4">
                    <div className="text-xs font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2 hidden md:flex">
                    ...
                    <div className="flex bg-slate-800 p-1 rounded-lg border border-slate-700">
                        ...
                        >
                            <Swords size={14} /> Matchup Predictor
                        </button>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                        <select
                            className="bg-slate-800 border border-slate-700 rounded px-3 py-1.5 text-sm font-bold text-blue-400 focus:outline-none focus:ring-1 focus:ring-blue-500"
                        ...
                        {isMatchupMode && (
                            <>
                                ...
                                <select
                                    className="bg-slate-800 border border-slate-700 rounded px-3 py-1.5 text-sm font-bold text-rose-400 focus:outline-none focus:ring-1 focus:ring-rose-500"
                                    value={selectedTeamB?.team_name || ''}
                                    onChange={(e) => {
                                        const team = teams.find(t => t.team_name === e.target.value);
                                        if (team) setSelectedTeamB(team);
                                    }}
                                >
                                    {teams.map(t => (
                ...
                <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 space-y-8 animate-in fade-in duration-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="bg-slate-900 border border-blue-500/30 rounded-2xl p-6 shadow-xl relative overflow-hidden">
                            ...
                            <h2 className="text-3xl font-black text-white mb-2">{selectedTeam.team_name}</h2>
                            <div className="flex gap-4 text-sm font-bold tracking-wider uppercase mb-6">
                                <span className="text-slate-400">NET <span className="text-blue-400">#{selectedTeam.profile?.net || '-'}</span></span>
                                <span className="text-slate-600">|</span>
                                <span className="text-slate-400">AdjEM <span className="text-white">+{selectedTeam.adj_em}</span></span>
                            </div>
                            <div className="grid grid-cols-3 gap-2 text-center">
                                <div className="bg-slate-950 p-2 rounded-lg border border-slate-800">
                                    <div className="text-[10px] sm:text-xs text-slate-500 font-bold mb-1">OFF</div>
                                    <div className={`text-sm sm:text-lg font-black ${getEfficiencyColor(selectedTeam.adj_o, false)}`}>{selectedTeam.adj_o}</div>
                                </div>
                                <div className="bg-slate-950 p-2 rounded-lg border border-slate-800">
                                    <div className="text-[10px] sm:text-xs text-slate-500 font-bold mb-1">DEF</div>
                                    <div className={`text-sm sm:text-lg font-black ${getEfficiencyColor(selectedTeam.adj_d, true)}`}>{selectedTeam.adj_d}</div>
                                </div>
                                <div className="bg-slate-950 p-2 rounded-lg border border-slate-800">
                                    <div className="text-[10px] sm:text-xs text-slate-500 font-bold mb-1">TEMPO</div>
                                    <div className="text-sm sm:text-lg font-black text-white">{selectedTeam.adj_t}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-slate-900 border border-rose-500/30 rounded-2xl p-6 shadow-xl relative overflow-hidden">
                            ...
                            <h2 className="text-3xl font-black text-white mb-2">{selectedTeamB?.team_name}</h2>
                            <div className="flex gap-4 text-sm font-bold tracking-wider uppercase mb-6">
                                <span className="text-slate-400">NET <span className="text-rose-400">#{selectedTeamB?.profile?.net || '-'}</span></span>
                                <span className="text-slate-600">|</span>
                                <span className="text-slate-400">AdjEM <span className="text-white">+{selectedTeamB?.adj_em}</span></span>
                            </div>
                            <div className="grid grid-cols-3 gap-2 text-center">
                                <div className="bg-slate-950 p-2 rounded-lg border border-slate-800">
                                    <div className="text-[10px] sm:text-xs text-slate-500 font-bold mb-1">OFF</div>
                                    <div className={`text-sm sm:text-lg font-black ${getEfficiencyColor(selectedTeamB?.adj_o, false)}`}>{selectedTeamB?.adj_o}</div>
                                </div>
                                <div className="bg-slate-950 p-2 rounded-lg border border-slate-800">
                                    <div className="text-[10px] sm:text-xs text-slate-500 font-bold mb-1">DEF</div>
                                    <div className={`text-sm sm:text-lg font-black ${getEfficiencyColor(selectedTeamB?.adj_d, true)}`}>{selectedTeamB?.adj_d}</div>
                                </div>
                                <div className="bg-slate-950 p-2 rounded-lg border border-slate-800">
                                    <div className="text-[10px] sm:text-xs text-slate-500 font-bold mb-1">TEMPO</div>
                                    <div className="text-sm sm:text-lg font-black text-white">{selectedTeamB?.adj_t}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-2xl relative">
                        {loadingMatchup ? (
                                ...
                                <div className="grid md:grid-cols-2 gap-8">
                                    <div className="bg-slate-950/50 p-6 rounded-xl border border-slate-800">
                                        <h4 className="text-sm font-bold text-blue-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                                            <TrendingUp size={16} /> {selectedTeam.team_name} Advantages
                                        </h4>
                                        <ul className="space-y-3">
                                            {matchupData.team_a_advantages?.map((adv, i) => (
                                    ...
                                    <div className="bg-slate-950/50 p-6 rounded-xl border border-slate-800">
                                        <h4 className="text-sm font-bold text-rose-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                                            <TrendingUp size={16} /> {selectedTeamB?.team_name} Advantages
                                        </h4>
                                        <ul className="space-y-3">
                                            {matchupData.team_b_advantages?.map((adv, i) => (
                                                <li key={i} className="flex gap-3 text-sm text-slate-300 items-start">
                            ...
                            <div className="text-center py-12 text-slate-500 font-mono">
                                Select opponent to generate matchup analysis.
                            </div>
                        )}
                    </div>
                </div>
