...
const BracketTable = ({ data, roundLabels }) => {
    ...
    Object.entries(data.regions || {}).forEach(([region, rounds = {}]) => {
        Object.entries(roundLabels).forEach(([roundKey, label]) => {
            (rounds[roundKey] || []).forEach((match, idx) => {
                const favoriteData = determineMatchFavorite(match);
                rows.push({
                    region,
                    round: label,
                    teamA: match.team_a,
                    teamB: match.team_b,
                    favorite: favoriteData.favoriteTeam,
                    favoritePct: favoriteData.favoritePct,
                    dogPct: favoriteData.dogPct,
    ...
    if (data.championship) {
        ...
        const finalFavorite = determineMatchFavorite(match);
        rows.push({
            region: 'Championship',
            round: 'Championship',
            teamA: match.team_a,
            teamB: match.team_b,
            favorite: finalFavorite.favoriteTeam,
            favoritePct: finalFavorite.favoritePct,
            dogPct: finalFavorite.dogPct,
            winProbA: Number(match.win_prob_a) || 0,
            winProbB: Number(match.win_prob_b) || 0,
